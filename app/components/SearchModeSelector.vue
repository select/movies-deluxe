<template>
  <div class="flex items-center gap-0.5 p-1 bg-theme-surface/30 rounded-xl relative group/modes">
    ...
    >
      ...
      <!-- Tooltip -->
      <div
        class="absolute top-full left-1/2 -translate-x-1/2 mt-3 px-3 py-2 bg-theme-surface border border-theme-border shadow-2xl rounded-xl text-xs w-56 pointer-events-none opacity-0 group-hover:opacity-100 transition-all duration-200 z-50 translate-y-2 group-hover:translate-y-0"
      >
        ...
        <!-- Arrow -->
        <div
          class="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-theme-surface border-t border-l border-theme-border rotate-45"
        ></div>
      </div>
    </button>
  </div>
