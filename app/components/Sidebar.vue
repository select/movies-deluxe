<template>
  ...
  <aside
    class="md:hidden fixed bottom-6 right-6 z-50 glass shadow-2xl rounded-full h-14 border-white/30 dark:border-gray-700/50"
  >
    <div class="flex items-center gap-2 px-2 h-full">
      ...
      <NuxtLink
        ...
        :class="{ 'bg-red-100 dark:bg-red-900/30': $route.path === '/liked' }"
      >
        <div class="i-mdi-heart text-2xl" :class="{ 'text-red-500': $route.path === '/liked' }" />
        <span
          v-if="likedCount > 0"
          class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full size-5 flex items-center justify-center font-medium"
        >
          {{ likedCount > 99 ? '99+' : likedCount }}
        </span>
      </NuxtLink>
      ...
      <div class="relative group">
        <button
          ...
          @click="openFilters"
        >
          <div class="i-mdi-filter-variant text-2xl" :class="{ 'text-blue-500': activeFiltersCount > 0 }" />
          <span
            v-if="activeFiltersCount > 0"
