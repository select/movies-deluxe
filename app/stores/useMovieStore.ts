...
export const useMovieStore = defineStore("movie", () => {
	...
	const fetchMovieDetails = async (identifier: string) => {
		...
		const response = await useFetch<MovieDetails>(
	...
	const saveMovieToStrapi = async (movie: Movie) => {
		...
		try {
			...
			let links = await find("external-links", {
				filters: {
					$and: [
						{ site: { $eq: "archive.org" } },
			...
			const year = movie.date
				? new Date(movie.date).getFullYear().toString()
				: undefined;
	...
	const getMovieDetails = async (id: string, imdbId: string) => {
		...
		const movieDetails = await $fetch<OmdbMovieDetails>(
		...
		const updateResult = await update("movies", id, {
			...
			imdbVotes:
				parseInt(movieDetails.imdbVotes.replace(/,/g, "")) || undefined,
		});
	...
	const getMovieImage = async (id: string, imdbId: string) => {
		...
		try {
			// Fetch movie images from OMDB API
			const movieImage = await $fetch<Blob>(
				`http://img.omdbapi.com/?apikey=${apiKey}&i=${imdbId}`,
			);
