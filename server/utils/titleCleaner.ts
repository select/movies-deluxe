...
export function cleanTitleGeneral(title: string): string {
  ...
  // 3. Remove possessive forms with quotes: Charlie Chaplin's "Title" → Title
  cleaned = cleaned.replace(/^[^"]+['']s\s*[""]([^""]+)[""].*$/, '$1')

  // 4. Remove actor/director promotional text: "ACTOR NAME is/in TITLE" → TITLE
  ...
  // 5. Remove pipe-separated promotional text (keep first part before pipe)
  ...
  // 6. Remove actor lists in parentheses: "Title ( Actor1, Actor2, Actor3 )"
  ...
  // Must be done BEFORE step 7 to avoid extracting actor names as titles
  cleaned = cleaned.replace(/\s*\(\s*[A-Z][^,)]+(?:,\s*[A-Z][^,)]+)+\s*\)\s*$/, '')

  // 7. Remove foreign language translations in parentheses (keep English if present)
  ...
  // 8. Remove metadata in parentheses: (German: ...), (1974, dir. Name)
  cleaned = cleaned.replace(
    /\s*\([^)]*(?:German|French|Spanish|Italian|Russian|dir\.|directed by)[^)]*\)/gi,
    ''
  )

  // 9. Remove genre/type descriptors in parentheses: (the grindhouse experience)
  cleaned = cleaned.replace(/\s*\([^)]*(?:experience|thriller|drama|comedy|horror)\s*\)/gi, '')

  // 10. Remove years in parentheses: (1999), (2000)
  cleaned = cleaned.replace(/\s*\(\s*\d{4}\s*\)\s*/g, ' ')

  // 11. Remove actor names from title: "Title - Actor Name (Year)"
  cleaned = cleaned.replace(/\s*[-–—]\s*[A-Z][a-z]+\s+[A-Z][a-z]+\s*\(\d{4}\)\s*$/, '')

  // 12. Remove descriptive subtitles after dash/colon (common in Archive.org)
  ...
  // 13. Remove quotes around entire title or parts
  cleaned = cleaned.replace(/^[""](.+)[""]$/, '$1')
  cleaned = cleaned.replace(/[""]([^""]+)[""]/, '$1')

  // 14. Remove trailing punctuation and descriptors
  cleaned = cleaned.replace(/[.,!?;:]\s*$/, '')

  // 15. Clean up whitespace
  cleaned = cleaned.replace(/\s+/g, ' ').trim()

  // 16. If cleaning resulted in empty string, return original
