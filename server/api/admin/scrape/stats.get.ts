...
export default defineEventHandler(async _event => {
  ...
  const emitStepProgress = (message: string) => {
    currentStep++
    emitProgress({
      type: 'stats',
      status: 'in_progress',
  ...
  try {
    emitProgress({
      ...
      current: 0,
      total: totalSteps,
      message: 'Starting stats refresh...',
    })

    emitStepProgress('Loading movies database...')
    ...
    emitStepProgress('Loading failed records...')
    ...
    emitStepProgress('Calculating poster statistics...')
    // Poster stats
    ...
    emitProgress({
      type: 'stats',
      status: 'error',
      message: `Stats refresh failed: ${error instanceof Error ? error.message : String(error)}`,
      current: 0,
      total: totalSteps,
    })
