 ...
 */
async function enrichMovies(options: EnrichmentOptions): Promise<EnrichmentResult> {
  ...
  const moviesToProcess = options.limit ? unmatchedMovies.slice(0, options.limit) : unmatchedMovies
  ...
  for (let i = 0; i < moviesToProcess.length; i++) {
    ...
    try {
      ...
      if (!meetsConfidenceThreshold(matchResult.confidence, options.minConfidence)) {
        logger.warn(`Match confidence too low (${matchResult.confidence}) for: ${movie.title}`)
        result.skipped++
        continue
      }
...
function printReport(result: EnrichmentResult, dryRun: boolean): void {
  ...
  if (result.matches.length > 0) {
    ...
    for (const match of result.matches) {
      logger.success(`${match.oldId} → ${match.newId}: ${match.title} [${match.confidence}]`)
    }
  }
