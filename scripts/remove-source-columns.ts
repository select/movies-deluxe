/**
 * Remove Unused Source Columns Migration Script
 *
 * Removes the label, quality, and fileSize columns from the sources table
 * since they are always empty (all 37,269 sources have NULL values).
 *
 * What it does:
 * - Creates a backup of the database
 * - Recreates the sources table without label, quality, fileSize columns
 * - Preserves all existing data (sources with their quality marks)
 ...
 *
 * Usage:
 *   pnpm tsx scripts/remove-source-columns.ts
 */
...
async function migrate() {
  log('🔧 Starting migration: Remove unused source columns')
  log('')
  ...
  try {
    ...
    const labelCount = db.prepare('SELECT COUNT(label) as count FROM sources').get() as {
      count: number
    }
    const qualityCount = db.prepare('SELECT COUNT(quality) as count FROM sources').get() as {
      count: number
    }
    const fileSizeCount = db.prepare('SELECT COUNT(fileSize) as count FROM sources').get() as {
      count: number
    }
    log(`  - Sources with label: ${labelCount.count}`)
    log(`  - Sources with quality: ${qualityCount.count}`)
    log(`  - Sources with fileSize: ${fileSizeCount.count}`)

    if (labelCount.count > 0 || qualityCount.count > 0 || fileSizeCount.count > 0) {
      logError(
        'Cannot proceed: Some sources have non-null values in columns to be removed. Please review.'
      )
      db.close()
      process.exit(1)
    }
    logSuccess('All columns are empty, safe to remove')
    log('')
    ...
    try {
      ...
      // Create new sources table without label, quality, fileSize columns
      ...
      // Create a temporary table to save quality marks with new IDs
