...
export async function searchOMDB(
  ...
  try {
    ...
    if (data.Response === "False") {
      logger.debug(
        `No results for "${title}" ${year ? `(${year})` : ""}: ${data.Error}`,
      );
    }
...
function calculateConfidence(
  ...
  if (
    ...
    Math.min(originalWords.length, resultWords.length) * 0.6
  ) {
    return "medium" as MatchConfidence;
  }
...
export async function matchMovie(
  ...
  if (!title || typeof title !== "string" || title.trim() === "") {
    logger.warn(
      `Invalid title provided to matchMovie: ${JSON.stringify(title)}`,
    );
    return {
