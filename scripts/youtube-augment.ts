...
const ANONYMIZED_METADATA_FILE = path.join(__dirname, '../data/anonymized-metadata.json')
...
function loadSongMetadata(youtubeId: string): SongMetaData | null {
  ...
  try {
    ...
    console.warn(`Error loading song metadata for ${youtubeId}:`, (error as Error).message)
    return null
  }
}
...
function saveSongMetadata(youtubeId: string, metadata: SongMetaData): boolean {
  ...
  try {
    ...
    console.error(`Error saving song metadata for ${youtubeId}:`, (error as Error).message)
    return false
  }
}
...
function loadAllVideoIds(): Set<string> {
  try {
    ...
    console.error('Error loading anonymized metadata:', (error as Error).message)
    process.exit(1)
  }
}
 ...
 */
function createNewSongMetadata(youtubeId: string, title: string, channel: string): SongMetaData {
  return {
    youtubeId,
    title,
...
async function fetchYouTubeMetadata(
  ...
  try {
    ...
    console.error(`Error fetching YouTube metadata for ${videoId}:`, (error as Error).message)
    return null
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`Existing files needing YouTube metadata: ${existingFilesNeedingUpdate.length}\n`)

  // Combine both lists
  const allVideosToProcess = [...videosNeedingNewFiles, ...existingFilesNeedingUpdate]
  ...
  console.log(`Total videos to process: ${allVideosToProcess.length}\n`)
  ...
  for (let i = 0; i < allVideosToProcess.length; i++) {
    ...
    if (saveSongMetadata(youtubeId, metadataToSave)) {
      ...
      console.log(`[${i + 1}/${allVideosToProcess.length}] ❌ ${youtubeId}: Failed to save`)
      errorCount++
    }
