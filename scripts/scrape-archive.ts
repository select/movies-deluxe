...
async function fetchArchiveOrgMovies(
  ...
  logger.info(
    `Fetching ${collection} page ${page + 1} (${start}-${start + rows})...`,
  );
...
async function processMovie(
  ...
  if (!options.skipOmdb && options.omdbApiKey) {
    ...
    if (matchResult.confidence !== "none" && matchResult.imdbId) {
      ...
      logger.success(
        `Matched "${movie.title}" to ${imdbId} (${matchResult.confidence} confidence)`,
      );
    } else {
      logger.warn(
        `No OMDB match for "${movie.title}", using temporary ID: ${imdbId}`,
      );
    }
  }
...
async function scrapeArchiveOrg(options: ScraperOptions): Promise<void> {
  ...
  for (const collection of options.collections) {
    ...
    for (let page = 0; page < options.pages; page++) {
      const movies = await fetchArchiveOrgMovies(
        collection,
        options.rows,
        page,
      );
...
const omdbApiKey =
  values["omdb-api-key"] || process.env.NUXT_PUBLIC_OMDB_API_KEY;

if (!values["skip-omdb"] && !omdbApiKey) {
  logger.warn(
    "OMDB API key not found. Set NUXT_PUBLIC_OMDB_API_KEY or use --omdb-api-key",
  );
  logger.warn(
    "Continuing without OMDB matching (use --skip-omdb to suppress this warning)",
  );
}
