...
function validateSources(
  ...
  movie.sources.forEach((source, index) => {
    ...
    if (source.type === "archive.org") {
      issues.push(
        ...validateArchiveSource(movieId, source as ArchiveOrgSource, index),
      );
    } else if (source.type === "youtube") {
      issues.push(
        ...validateYouTubeSource(movieId, source as YouTubeSource, index),
      );
    } else {
      issues.push({
        severity: "error",
...
function fixIssue(db: MoviesDatabase, issue: ValidationIssue): boolean {
  ...
  try {
    ...
    if (
      ...
      issue.message.includes("Duplicate source URLs")
    ) {
      const uniqueSources = Array.from(
        new Map(movie.sources.map((s) => [s.url, s])).values(),
      );
...
async function validateDatabase(
  ...
  report.warnings = report.issues.filter(
...
async function main() {
  ...
  if (values.fix) {
    logger.warn(
      "Fix mode enabled - issues will be auto-corrected where possible",
    );
  }
